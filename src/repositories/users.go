...
func (repository usersRepository) GetPassword(userId uint64) (string, error) {
	...
	if row.Next() {
		if error := row.Scan(&password); error !=nil {
			return "", error
		}
	}
...
func (repository usersRepository) UpdatePassword(userId uint64, newPassword string) error {
